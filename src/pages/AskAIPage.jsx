...
const quickActions = ["Explain simpler", "Give example", "Generate flashcards", "Create practice question", "Summarize this lecture"];
          ...
          >
            {modules.map((module) => (
              <option key={module.id} value={module.id}>
                Lecture {module.position}: {module.title}
              </option>
            ))}
          </select>
        </div>
      ...
      <section className="soft-card p-5">
        ...
        <div className="mt-3 rounded-[20px] bg-navy p-4 text-white">
          ...
          <p className="mt-1 text-xs font-bold text-white/65">
            {module ? `Lecture ${module.position}: ${module.title}` : "Create a course to start"}
          </p>
        </div>
      </section>
      <section className="soft-card p-5">
